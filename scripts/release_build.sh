...
for p in "${platforms[@]}"; do
  ...
  GOOS="$goos" GOARCH="$goarch" CGO_ENABLED="$cgo_enabled" CC="$cc" go build -trimpath -ldflags="-s -w -X github.com/shufo/gh-issue-stats/cmd.Version=${VERSION}" -o "dist/${p}${ext}"
done
